...
module "analytics" {
  ...
  enable_firehose_transformation = var.enable_firehose_transformation
...
# DNS Module (Route53 + ACM Certificates)
module "dns" {
  source = "./modules/dns"
  ...
  # Use specific certificates for better security (not wildcard)
...
module "admin_portal" {
  ...
  # Use domain from DNS module
  domain_name         = module.dns.admin_domain
  ...
  # WAF (optional)
  waf_web_acl_arn = var.enable_waf_cloudfront ? module.waf.cloudfront_web_acl_arn : ""
